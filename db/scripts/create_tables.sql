...
CREATE TABLE cart_session (
    ...
    user_identifier VARCHAR(255) UNIQUE NOT NULL, -- Could be guest ID or user ID
    created_at TIMESTAMP WITH TIME ZONE DEFAULT current_timestamp,
    last_modified TIMESTAMP WITH TIME ZONE DEFAULT current_timestamp,
    metadata jsonb -- For storing additional session info like device, browser etc.
);
...
CREATE TABLE cart_items (
    ...
    UNIQUE(session_id, product_id) -- Prevent duplicate products in cart
);

-- Enhanced ORDER table with Stripe-specific fields
CREATE TABLE "ORDER" (
...
-- Enhanced PAYMENT table with Stripe-specific fields
